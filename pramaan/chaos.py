...
from pramaan.bq import execute_query, execute_query_job, get_bq_client

GROUND_TRUTH_LOG = os.getenv("GROUND_TRUTH_LOG", ".ground_truth.json")
SNAPSHOT_PREFIX = "_snap_"

def log_ground_truth(scenario: str, table: str, target_column: str, expected_rule_type: str, run_id: str):
...
def inject_null_flood(dataset: str, table: str = "users", column: str = "email", run_id: Optional[str] = None) -> Dict[str, Any]:
    """Scenario 1: Injects NULLs to trigger null_rate rule."""
    _ensure_snapshot(dataset, table)
    run_id = _resolve_run_id(run_id)
    entry = log_ground_truth("null_flood", table, column, "null_rate", run_id)
    query = f"""
...
def inject_silent_duplicate_load(dataset: str, table: str = "orders", column: str = "order_id", run_id: Optional[str] = None) -> Dict[str, Any]:
    """Scenario 2: Duplicates primary keys to trigger uniqueness rule."""
    _ensure_snapshot(dataset, table)
    run_id = _resolve_run_id(run_id)
    entry = log_ground_truth("silent_duplicate_load", table, column, "uniqueness", run_id)
    query = f"""
...
def inject_currency_swap(dataset: str, table: str = "order_items", column: str = "sale_price", run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    _ensure_snapshot(dataset, table)
    run_id = _resolve_run_id(run_id)
    entry = log_ground_truth("currency_swap", table, column, "range", run_id)
    query = f"""
    UPDATE `{dataset}.{table}`
...
def inject_stalled_partition(dataset: str, table: str = "orders", column: str = "created_at", run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    _ensure_snapshot(dataset, table)
    run_id = _resolve_run_id(run_id)
    entry = log_ground_truth("stalled_partition", table, column, "freshness", run_id)
    query = f"""
    UPDATE `{dataset}.{table}`
...
def inject_referential_orphan(dataset: str, table: str = "order_items", column: str = "user_id", run_id: Optional[str] = None) -> Dict[str, Any]:
    """Scenario 5: Injects orphan IDs to trigger referential_integrity rule."""
    _ensure_snapshot(dataset, table)
    run_id = _resolve_run_id(run_id)
    entry = log_ground_truth("referential_orphan", table, column, "referential_integrity", run_id)
    query = f"""
    UPDATE `{dataset}.{table}`
    SET {column} = 99999999
    WHERE RAND() < 0.10;
...
def inject_schema_drift(dataset: str, table: str = "order_items", column: str = "status", run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    _ensure_snapshot(dataset, table)
    run_id = _resolve_run_id(run_id)
    entry = log_ground_truth("schema_drift", table, column, "schema_conformance", run_id)
    query = f"""
